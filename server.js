...
app.post("/auto-fill-unique-ids", async (req, res) => {
  ...
  if (!activeSpreadsheetId) {
    return res
      .status(400)
      .json({ success: false, message: "Active spreadsheet ID is required." });
  }
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "No data found in the main spreadsheet",
        });
    }
    ...
    if (uniqueIdColumnIndex === -1) {
      return res
        ...
        .json({
          success: false,
          message: "Unique ID column not found in the spreadsheet",
        });
    }
    ...
    for (let i = 1; i < rows.length; i++) {
      const row = rows[i];
      if (
        !row[uniqueIdColumnIndex] &&
        row.some((cell, index) => index !== uniqueIdColumnIndex && cell)
      ) {
        lastUniqueId += 1;
        updates.push({
          range: `Sheet1!${String.fromCharCode(65 + uniqueIdColumnIndex)}${
            i + 1
          }`,
          values: [[lastUniqueId]],
        });
    ...
    res
      .status(200)
      .json({ success: true, message: "Unique IDs filled successfully" });
  } catch (err) {
    console.error("Error filling unique IDs:", err.message);
    res
      .status(500)
      .json({ success: false, message: "Failed to fill unique IDs" });
  }
});

// Handle Registration
app.post("/register", async (req, res) => {
  const { firstName, middleName, surname, mobile, email, gender, password } =
    req.body;
...
app.post("/forgot-password", async (req, res) => {
  ...
  try {
    ...
    if (user) {
      const token = jwt.sign({ email: user[4] }, SECRET_KEY, {
        expiresIn: "15m",
...
app.post("/reset-password", async (req, res) => {
  ...
  try {
    ...
    const user = rows.find(
      (row) => row[4]?.toLowerCase() === decoded.email.toLowerCase()
    );
  ...
  try {
    ...
    if (!headers || headers.length === 0) {
      return res
        ...
        .json({
          success: false,
          message: "No headers found in the active spreadsheet.",
        });
    }
    ...
    if (!existingSheets.includes("UnsubscribedUsers")) {
      ...
      await sheets.spreadsheets.batchUpdate({
        ...
        resource: {
          requests: [
            { addSheet: { properties: { title: "UnsubscribedUsers" } } },
          ],
        },
      });
    ...
    const firstSheetHeaders = firstSheetData.data.values
      ? firstSheetData.data.values[0]
      : [];
    const normalizedHeaders = firstSheetHeaders.map((h) =>
      h.toLowerCase().replace(/[^a-z0-9]/g, "")
    );
    const uniqueIdVariations = [
      "uniqueid",
      "unique_id",
      "unique-id",
      "unique.id",
    ...
    if (!hasUniqueIdColumn) {
      ...
      const newColumnLetter = String.fromCharCode(
        65 + firstSheetHeaders.length
      );
      await sheets.spreadsheets.values.update({
        spreadsheetId,
    ...
    const userRowIndex = sheet1Rows.findIndex(
      (row) => row[7] === user.uniqueID
    );

    if (userRowIndex === -1) {
      return res
        .status(404)
        .json({ success: false, message: "User row not found in Sheet1." });
    }
    ...
    await sheets.spreadsheets.values.update({
      ...
      resource: {
        values: [
          [
            existingIds ? `${existingIds},${spreadsheetId}` : spreadsheetId,
            existingNames
              ? `${existingNames},${spreadsheetName}`
              : spreadsheetName,
          ],
        ],
      },
    ...
    const userSpreadsheetRowIndex = spreadsheetIdRows.findIndex(
      (row) => row[0] === user.uniqueID
    );
    ...
    if (userSpreadsheetRowIndex === -1) {
      ...
      const existingUserIds =
        spreadsheetIdRows[userSpreadsheetRowIndex][1] || "";
      const existingUserNames =
        spreadsheetIdRows[userSpreadsheetRowIndex][2] || "";

      await sheets.spreadsheets.values.update({
        spreadsheetId: REGISTRATION_SPREADSHEET_ID,
        range: `SpreadSheetID!A${userSpreadsheetRowIndex + 1}:C${
          userSpreadsheetRowIndex + 1
        }`,
        valueInputOption: "RAW",
        resource: {
          values: [
            [
              user.uniqueID,
              existingUserIds
                ? `${existingUserIds},${spreadsheetId}`
                : spreadsheetId,
              existingUserNames
                ? `${existingUserNames},${spreadsheetName}`
                : spreadsheetName,
            ],
          ],
        },
    ...
    res
      ...
      .json({
        success: true,
        message: "Spreadsheet initialized and updated successfully.",
      });
  } catch (error) {
    console.error("Error initializing spreadsheet:", error);
    res
      .status(500)
      .json({ success: false, message: "Failed to initialize spreadsheet." });
  }
});
...
app.get("/get-active-spreadsheet", verifyToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch active spreadsheet." });
  }
});
...
app.get("/get-spreadsheet-headers", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!headers || headers.length === 0) {
      return res
        .status(404)
        .json({ message: "No headers found in the spreadsheet." });
    }
...
app.get("/get-spreadsheet-data", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!values.length) {
      return res
        .status(404)
        .json({ message: "No data found in the spreadsheet." });
    }
...
app.get("/get-spreadsheets", verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No spreadsheets found." });
    }
    ...
    if (!userRow) {
      return res
        ...
        .json({
          success: false,
          message: "No spreadsheets found for the user.",
        });
    }
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch spreadsheets." });
  }
});
...
app.post("/set-active-spreadsheet", verifyToken, async (req, res) => {
  ...
  if (!spreadsheetId) {
    return res
      .status(400)
      .json({ success: false, message: "Spreadsheet ID is required." });
  }
  ...
  try {
    ...
    res
      ...
      .json({
        success: true,
        message: "Active spreadsheet updated successfully.",
      });
  } catch (err) {
    console.error("Error setting active spreadsheet:", err);
    res
      .status(500)
      .json({ success: false, message: "Failed to set active spreadsheet." });
  }
});
...
app.post("/remove-spreadsheet", verifyToken, async (req, res) => {
  ...
  try {
    ...
    const updatedSpreadsheetNames = existingSpreadsheetNames
      ...
      .filter(
        (_, index) =>
          existingSpreadsheetIds.split(",")[index].trim() !== spreadsheetId
      )
      .join(",");
    ...
    if (userSpreadsheetRowIndex !== -1) {
      // Remove the spreadsheet ID and name from the lists
      const existingSpreadsheetIds =
        spreadsheetIdRows[userSpreadsheetRowIndex][1] || ""; // Column B (index 1)
      const existingSpreadsheetNames =
        spreadsheetIdRows[userSpreadsheetRowIndex][2] || ""; // Column C (index 2)
      ...
      await sheets.spreadsheets.values.update({
        ...
        range: `SpreadSheetID!A${userSpreadsheetRowIndex + 1}:C${
          userSpreadsheetRowIndex + 1
        }`, // Update columns A, B, and C
        valueInputOption: "RAW",
        resource: {
          values: [
            [user.uniqueID, updatedSpreadsheetIds, updatedSpreadsheetNames],
          ],
        },
      });
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "Spreadsheet removed successfully." });
  } catch (err) {
    console.error("Error removing spreadsheet:", err);
    res
      .status(500)
      .json({ success: false, message: "Failed to remove spreadsheet." });
  }
});
...
app.get("/fetch-registrations", verifyToken, async (req, res) => {
  if (!activeSpreadsheetId) {
    return res
      .status(400)
      .json({ success: false, message: "No active spreadsheet set." });
  }
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No data found in the spreadsheet." });
    }
    ...
    res
      ...
      .json({
        success: false,
        message: "Error fetching data from the spreadsheet",
      });
  }
});
...
app.post("/edit-profile", verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Find the user with the matching email
    const userRowIndex =
      rows.slice(1).findIndex((row) => row[4] === userEmail) + 1; // +1 to account for header row
...
app.post("/edit-row", async (req, res) => {
  ...
  if (!uniqueId || !updatedRow || !activeSpreadsheetId) {
    return res
      .status(400)
      .json({ success: false, message: "Invalid input provided." });
  }
  ...
  try {
    ...
    const userRowIndex = rows.findIndex(
      (row) => row[uniqueIdColumnIndex] === uniqueId.toString()
    );
...
app.post("/add-new-row", async (req, res) => {
  ...
  if (!newRowData || !activeSpreadsheetId) {
    return res
      ...
      .json({
        success: false,
        message: "New row data and active spreadsheet ID are required.",
      });
  }
  ...
  try {
    ...
    const isNonEditableHeader = (header) => {
      ...
      return (
        lowerCaseHeader.includes("unique") || lowerCaseHeader.includes("group")
      );
    };
    ...
    for (const header of headers) {
      if (isNonEditableHeader(header) && newRowData[header]) {
        return res
          ...
          .json({
            success: false,
            message: `Cannot modify non-editable header: ${header}`,
          });
      }
    }
    ...
    res
      .status(200)
      .json({ success: true, message: "New row added successfully." });
  } catch (err) {
...
app.delete("/delete-user", verifyToken, async (req, res) => {
  ...
  if (!uniqueId || !activeSpreadsheetId) {
    return res
      ...
      .json({
        success: false,
        message: "Unique ID and active spreadsheet ID are required.",
      });
  }
  ...
  try {
    ...
    const userRowIndex = mainSheetRows.findIndex(
      (row) => row[uniqueIdColumnIndex] === uniqueId.toString()
    );
    ...
    if (userRowIndex === -1) {
      return res
        .status(404)
        .json({ success: false, message: "User not found in the main sheet." });
    }
    ...
    if (!groupSheetRows || groupSheetRows.length === 0) {
      return res
        .status(404)
        .json({ success: false, message: "No data found in the group sheet." });
    }
    ...
    for (let i = 1; i < groupSheetRows.length; i++) {
      ...
      if (updatedMembers !== groupSheetRows[i][membersColumnIndex]) {
        ...
        await sheets.spreadsheets.values.update({
          spreadsheetId: activeSpreadsheetId,
          range: `Group!${String.fromCharCode(65 + membersColumnIndex)}${
            i + 1
...
app.delete("/delete-multiple-users", verifyToken, async (req, res) => {
  ...
  if (!uniqueIds || uniqueIds.length === 0 || !activeSpreadsheetId) {
    return res
      ...
      .json({
        success: false,
        message: "Unique IDs and active spreadsheet ID are required.",
      });
  }
  ...
  try {
    ...
    // Step 2: Dynamically identify the Unique ID column
    const uniqueIdColumnIndex = headers.findIndex(
      (header) =>
        header.toLowerCase().includes("unique") ||
    ...
    for (let i = 1; i < groupSheetRows.length; i++) {
      ...
      const groupMembers = groupSheetRows[i][membersColumnIndex].split(","); // Split members by comma
      const updatedMembers = groupMembers
        .filter((member) => !uniqueIds.includes(member.trim()))
        .join(",");
    ...
    res
      ...
      .json({
        success: true,
        message: "Users deleted successfully and removed from all groups.",
      });
  } catch (err) {
...
app.post("/create-group", async (req, res) => {
  const { groupName, description, selectedFields, activeSpreadsheetId } =
    req.body;
  ...
  if (
    ...
    return res
      ...
      .json({
        message:
          "Group name, selected fields, and active spreadsheet ID are required.",
      });
  }
  ...
  try {
    ...
    const sheetTitles = spreadsheetMetadata.data.sheets.map(
      (sheet) => sheet.properties.title
    );
    ...
    if (uniqueIdColumnIndex === -1) {
      ...
      return res
        .status(400)
        .json({ message: "Unique ID column not found in the spreadsheet." });
    }
    ...
    const groupMembers = selectedFields
      .map((field) => {
        if (!field.uniqueId) {
      ...
      .filter(Boolean)
      .join(","); // Use 'uniqueId' to get member IDs
    ...
    for (let field of selectedFields) {
      ...
      const userRowIndex = rows.findIndex((row) => {
        return (
          row[uniqueIdColumnIndex] &&
          field.uniqueId &&
          row[uniqueIdColumnIndex].toString().trim() ===
            field.uniqueId.toString().trim()
        );
      });
      ...
      if (userRowIndex === -1) {
        console.error(
          `User with unique ID ${field.uniqueId} not found in Sheet1.`
        );
      ...
      const updateRange = `Sheet1!${String.fromCharCode(
        65 + groupNameColumnIndex
      )}${userRowIndex + 1}`; // Convert index to column letter (e.g., 0 -> A, 1 -> B)

      // Append the group name to the existing value (if any)
      const currentGroupName = rows[userRowIndex][groupNameColumnIndex] || "";
      const updatedGroupName = currentGroupName
...
app.post("/delete-groups", verifyToken, async (req, res) => {
  ...
  if (!groupNames || groupNames.length === 0 || !activeSpreadsheetId) {
    return res
      ...
      .json({
        success: false,
        message: "Group names and active spreadsheet ID are required.",
      });
  }
  ...
  try {
    ...
    const groupNameColumnIndex = headers.findIndex(
      (header) =>
    ...
    if (groupNameColumnIndex === -1) {
      return res
        ...
        .json({
          success: false,
          message: "Group Name column not found in the main spreadsheet.",
        });
    }

    // Step 3: Update the main sheet (Sheet1) to remove the deleted groups from users
    for (let i = 1; i < rows.length; i++) {
      const userGroups = rows[i][groupNameColumnIndex]
        ? rows[i][groupNameColumnIndex].split(",")
        : [];
      const updatedGroups = userGroups
        .filter((group) => !groupNames.includes(group.trim()))
        .join(",");

      if (updatedGroups !== rows[i][groupNameColumnIndex]) {
        await sheets.spreadsheets.values.update({
          spreadsheetId: activeSpreadsheetId,
          range: `Sheet1!${String.fromCharCode(65 + groupNameColumnIndex)}${
            i + 1
          }`,
          valueInputOption: "USER_ENTERED",
          resource: {
    ...
    const groupNameColumnIndexGroupSheet = groupHeaders.findIndex(
      (header) =>
        header.toLowerCase().includes("group") &&
        header.toLowerCase().includes("name")
    );
    ...
    if (groupNameColumnIndexGroupSheet === -1) {
      return res
        ...
        .json({
          success: false,
          message: "Group Name column not found in the group sheet.",
        });
    }

    // Step 6: Filter out the rows corresponding to the selected groups
    const updatedGroupSheetRows = groupSheetRows.filter(
      (row) => !groupNames.includes(row[groupNameColumnIndexGroupSheet])
    );
    ...
    res
      ...
      .json({
        success: true,
        message: "Groups deleted successfully from both sheets.",
      });
  } catch (err) {
    console.error("Error deleting groups:", err.message);
    res
      .status(500)
      .json({ success: false, message: "Failed to delete groups." });
  }
});
app.get("/fetch-groups", async (req, res) => {
  ...
  try {
    ...
    const groupIdColumnIndex = headers.findIndex(
      (header) =>
        header.toLowerCase().includes("group") &&
...
app.post("/fetch-group-users", async (req, res) => {
  ...
  if (!groupNames || groupNames.length === 0 || !activeSpreadsheetId) {
    return res
      .status(400)
      .json({ message: "Group names and active spreadsheet ID are required." });
  }
  ...
  try {
    ...
    if (groupNameColumnIndex === -1) {
      ...
      return res
        .status(400)
        .json({ message: "Group Name column not found in Sheet1." });
    }
    ...
    const users = rows.slice(1).filter((row) => {
      const groupNamesInRow = row[groupNameColumnIndex]
        ?.split(",")
        .map((name) => name.trim());
      return groupNames.some((groupName) =>
        groupNamesInRow?.includes(groupName)
      );
    });
...
app.post("/combine-groups", async (req, res) => {
  const { groupNames, newGroupName, description, activeSpreadsheetId } =
    req.body;
  ...
  try {
    ...
    if (uniqueIdColumnIndex === -1 || groupNameColumnIndex === -1) {
      return res
        ...
        .json({
          success: false,
          message:
            "Unique ID or Group Name column not found in the spreadsheet.",
        });
    }
    ...
    groupSheetRows.slice(1).forEach((row) => {
      if (groupNames.includes(row[1])) {
        // Assuming group names are in column B (index 1)
        const userIds = row[membersColumnIndex].split(","); // Split members by comma
        userIds.forEach((id) => {
          const user = rows.find(
            (userRow) => userRow[uniqueIdColumnIndex] === id.trim()
          );
    ...
    const groupMembers = selectedUsers
      .map((user) => user[uniqueIdColumnIndex])
      .join(",");
    ...
    for (const user of selectedUsers) {
      const userGroups = user[groupNameColumnIndex]
        ? user[groupNameColumnIndex].split(",")
        : [];
      if (!userGroups.includes(newGroupName)) {
        ...
        const userRowIndex = rows.findIndex(
          (row) => row[uniqueIdColumnIndex] === user[uniqueIdColumnIndex]
        );
        await sheets.spreadsheets.values.update({
          spreadsheetId: activeSpreadsheetId,
    ...
    res
      .status(200)
      .json({ success: true, message: "Groups combined successfully." });
  } catch (err) {
    console.error("Error combining groups:", err.message);
    res
      .status(500)
      .json({ success: false, message: "Failed to combine groups." });
  }
});
...
app.post("/add-to-existing-groups", async (req, res) => {
  ...
  if (
    ...
    return res
      ...
      .json({
        message:
          "Group names, selected fields, and active spreadsheet ID are required.",
      });
  }
  ...
  try {
    ...
    const groupNameColumnIndex = groupHeaders.findIndex(
      (header) =>
        header.toLowerCase().includes("group") &&
        header.toLowerCase().includes("name")
    );
    ...
    if (groupNameColumnIndex === -1 || membersColumnIndex === -1) {
      return res
        .status(400)
        .json({
          message: "Group Name or Members column not found in the Group sheet.",
        });
    }
    ...
    const groupNameColumnIndexSheet1 = userHeaders.findIndex(
      (header) =>
        header.toLowerCase().includes("group") &&
        header.toLowerCase().includes("name")
    );
    ...
    if (uniqueIdColumnIndex === -1 || groupNameColumnIndexSheet1 === -1) {
      return res
        .status(400)
        .json({
          message: "Unique ID or Group Name column not found in Sheet1.",
        });
    }

    console.log("Unique ID column index in Sheet1:", uniqueIdColumnIndex);
    console.log(
      "Group Name column index in Sheet1:",
      groupNameColumnIndexSheet1
    );

    // Step 5: Update each selected group with the new users
    for (const groupName of groupNames) {
      const groupRowIndex = groupRows.findIndex(
        (row) => row[groupNameColumnIndex] === groupName
      );
      if (groupRowIndex !== -1) {
        const groupRow = groupRows[groupRowIndex];
        const existingMembers = groupRow[membersColumnIndex]
          ? groupRow[membersColumnIndex].split(",")
          : [];
        const newMembers = selectedFields.map((field) =>
          field.uniqueId.toString().trim()
        ); // Ensure uniqueId is a string and trimmed
    ...
    for (const user of selectedFields) {
      const userRowIndex = userRows.findIndex(
        (row) =>
          row[uniqueIdColumnIndex] &&
          row[uniqueIdColumnIndex].toString().trim() ===
            user.uniqueId.toString().trim()
      );

      console.log(
        `User row index for uniqueId ${user.uniqueId}:`,
        userRowIndex
      );

      if (userRowIndex !== -1) {
        const userRow = userRows[userRowIndex];
        const existingGroups = userRow[groupNameColumnIndexSheet1]
          ? userRow[groupNameColumnIndexSheet1].split(",")
          : [];
        const updatedGroups = [
          ...new Set([...existingGroups, ...groupNames]),
        ].join(",");

        console.log(
          `Updating groups for user ${user.uniqueId}:`,
          updatedGroups
        );
        ...
        await sheets.spreadsheets.values.update({
          ...
          range: `Sheet1!${String.fromCharCode(
            65 + groupNameColumnIndexSheet1
          )}${userRowIndex + 1}`, // Convert index to column letter
          valueInputOption: "USER_ENTERED",
          resource: {
            values: [[updatedGroups]],
          },
        ...
        console.error(
          `User with uniqueId ${user.uniqueId} not found in Sheet1.`
        );
      }
    }
    ...
    res
      ...
      .json({
        success: true,
        message: "Users added to existing groups successfully",
      });
  } catch (err) {
    console.error("Error adding users to existing groups:", err.message);
    res
      .status(500)
      .json({ message: "Failed to add users to existing groups." });
  }
});
...
app.post("/create-template", async (req, res) => {
    const url = `https://graph.facebook.com/v19.0/${BUSINESS_ACCOUNT_ID}/message_templates`;
  
    const templatePayload = {
      name: "order_update_notificationss", // Unique template name
      category: "UTILITY",  // ✅ Fixed: Use UTILITY for transactional updates
      language: "en_US",
      components: [
        {
          type: "HEADER",
          format: "TEXT",
          text: "Order Update"  // ✅ Fixed: Properly formatted placeholder
        },
        {
          type: "BODY",
          text: "Hello {{1}}, your order {{2}} has been confirmed! It is expected to arrive by {{3}}. You can track your order here"
        },
        {
          type: "FOOTER",
...
// Get Template Status
app.get("/get-template-status/:template_name", async (req, res) => {
  ...
  try {
    ...
    res.json({ name: template.name, status: template.status, id: template.id });
  } catch (error) {
    console.error(error.response ? error.response.data : error.message);
    res
      .status(500)
...
app.get("/get-all-templates", async (req, res) => {
  ...
  try {
    ...
    const formattedTemplates = response.data.data.map((template) => ({
      messaging_product: "whatsapp",
      type: "template",
      category: template.category || "utility", // Add a default category if not present
      template: {
        name: template.name,
        language: {
              ...
              if (["HEADER", "BODY", "FOOTER"].includes(component.type)) {
                return {
                  ...
                  parameters: [
                    {
                      type: "text",
                      text: component.text || "" // Ensure text is present
                    }
                  ]
                };
              }
              return component; // Keep other components unchanged
            })
          : []
      }
    }));
...
app.put("/edit-template/:template_id/:template_name/", async (req, res) => {
  const templateName = req.params.template_name;
  const templateId = req.params.template_id;

  const deleteUrl = `https://graph.facebook.com/v17.0/${BUSINESS_ACCOUNT_ID}/message_templates?name=${templateName}&hsm_id=${templateId}`;

  try {
    // Step 1: Delete the existing template
    console.log("Deleting template with URL:", deleteUrl);
    ...
    const createUrl = `https://graph.facebook.com/v17.0/${BUSINESS_ACCOUNT_ID}/message_templates`;

    const updatedTemplatePayload = {
      name: "order_tracking", // Use a simple name for testing
      category: "UTILITY",
      language: "en_US",
      components: [
        {
          type: "BODY",
          text: "Your order #{{1}} has been shipped and will arrive by {{2}}.", // Simple text for testing
        },
      ],
    };

    console.log("Creating template with payload:", updatedTemplatePayload);
    const response = await axios.post(createUrl, updatedTemplatePayload, {
      headers: {
        Authorization: `Bearer ${ACCESS_TOKEN}`,
        "Content-Type": "application/json",
      },
  ...
  app.delete("/delete-template/:template_id/:template_name/", async (req, res) => {
    const templateName = req.params.template_name;
    console.log("tem name", req.params);
    const templateId = req.params.template_id;
    const url = `https://graph.facebook.com/v17.0/${BUSINESS_ACCOUNT_ID}/message_templates?name=${templateName}&hsm_id=${templateId}`;
    ...
    try {
      ...
      console.error(error.response ? error.response.data : error.message);
      if (error.response && error.response.data.error.code === 100) {
        res.status(404).json({
          error: "Object not found or cannot be deleted.",
...
app.get("/get-unsubscribed-users", async (req, res) => {
  try {
    ...
    const headers = headerResponse.data.values
      ? headerResponse.data.values[0].map((h) => h.toLowerCase())
      : [];
    if (!headers.length) {
    ...
    if (phoneIndex === -1) {
      console.log(
        "Phone-related column not found in 'UnsubscribedUsers' sheet."
      );
      return res
        .status(400)
        .json({ success: false, message: "Phone column not found." });
    }
    ...
    const rows = response.data.values || [];
    if (rows.length < 2) {
      // Only headers exist, no unsubscribed users
    ...
    const unsubscribedPhones = rows
      .slice(1)
      .map((row) => row[phoneIndex]?.trim())
      .filter(Boolean);
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch unsubscribed users." });
  }
});
...
(async () => {
  ...
  app.post("/send-telegram", upload.array("files"), async (req, res) => {
    ...
    if (
      (!message || !parsedRecipients || parsedRecipients.length === 0) &&
      (!files || files.length === 0)
    ) {
      return res
        .status(400)
        .json({
          error: "Message or files and recipient details are required.",
        });
    }
    ...
    try {
      ...
      await Promise.all(
        validRecipients.map(async (recipient) => {
          ...
          try {
            ...
            console.error(
              `Failed to send message to ${recipient.phone}: ${error.message}`
            );
            results.push({
              ...recipient,
              status: "failed",
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to send Telegram messages." });
    }
  });
})();
...
app.post("/send-sms", upload.array("files"), async (req, res) => {
  ...
  const isTestMessage =
    parsedRecipients.length === 1 && parsedRecipients[0].uniqueId === "test";
  ...
  if (
    ...
    return res
      .status(400)
      .json({ error: "Message or files and recipient details are required." });
  }
    ...
    const results = await Promise.all(
      parsedRecipients.map(async (recipient) => {
        ...
        try {
          ...
          if (files && files.length > 0) {
            messageOptions.mediaUrl = files.map(
              (file) => `file://${file.path}`
            );
          }
  ...
  try {
    ...
    for (const recipient of recipients) {
      for (const key of phoneNumberKeys) {
        if (
          recipient[key] &&
          normalizePhone(recipient[key]) === normalizedMessagePhone
        ) {
    ...
    if (!unsubscribedUser) {
      console.error(
        `User with phone number ${normalizedMessagePhone} not found in recipients.`
      );
      return;
    }
    ...
    console.log(
      `User ${normalizedMessagePhone} successfully added to 'UnsubscribedUsers'.`
    );
      ...
      entry.changes.forEach((change) => {
        ...
        if (value.messages) {
          value.messages.forEach(async (message) => {
            ...
            if (
              message.type === "button" &&
              message.button.text.toLowerCase() === "unsubscribe"
            ) {
              ...
              const recipients = await getRecipientsFromSpreadsheet(
                activeSpreadsheetId,
                userPhone
              );
